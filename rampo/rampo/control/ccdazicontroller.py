...
class CCDAziController(object):
    ...
    def integrate_to_1d(self):
        ...
        if azi_list is None:
            ...
            return None
        # Integration uses the loaded CCD image directly.
        if (not hasattr(self.model, "diff_img")) or (self.model.diff_img is None):
            QtWidgets.QMessageBox.warning(
                self.widget, 'Warning',
                'No CCD image object is available for integration.')
            return None
        ...
        if getattr(self.model.diff_img, "img", None) is None:
            ...
            QtWidgets.QMessageBox.warning(
                self.widget, 'Warning',
                'CCD image is not loaded. '
                'Integration requires the source image data.\n'
                'Move the image file into the same folder as the spectrum.' +
                expected_text)
            return None
        self.ccdprocess_ctrl.read_settings()
        x_axis = []
        intensity = []
        mid_angle = self.widget.spinBox_AziShift.value()
        n_rows = self.model.diff_img.img.shape[0]
        shift = int(mid_angle) % n_rows if n_rows > 0 else 0
        for azi_i in azi_list:
            ...
            try:
                x_i, intensity_i = self.model.diff_img.integrate_to_1d(
        ...
        for x_i, intensity_i in zip(x_axis, intensity):
            if not np.array_equal(x_i, x_axis[0]):
                QtWidgets.QMessageBox.warning(
                    self.widget, 'Warning',
                    'Error occured while preparing for CCD-row ' + \
        ...
        preheader_line1 = 'Raman shift / CCD x\n'
        preheader_line2 = '\n'
        writechi(filen_chi, x_axis[0], intensity_output,
...
CakeAziController = CCDAziController
