...
class CCDImage(object):
    def __init__(self):
        ...
        # mask for self.img, not self.intensity_ccd (ccd image)
        self.mask = None 
        # self.intensity is for intersity of 1D pattern
        self.intensity = None
        self.tth = None
        # the following three are for ccd
        self.intensity_ccd = None
        self.tth_ccd = None
        self.chi_ccd = None
    ...
    def _set_direct_image_axes(self, x_axis=None):
        ...
        self.tth_ccd = x_axis
        self.chi_ccd = y_axis
        self.intensity_ccd = np.asarray(self.img, dtype=float)
        self.tth = x_axis
        self.intensity = np.nansum(self.intensity_ccd, axis=0)
    ...
    def apply_excitation_wavelength(self, laser_wavelength_nm, use_wavenumber=True):
        ...
        self.tth = np.asarray(x_disp, dtype=float)
        self.tth_ccd = np.asarray(x_disp, dtype=float)
        return True
    ...
    def integrate_to_ccd(self, **kwargs):
        if self.img is None:
            raise ValueError("CCD image is not loaded.")
        t_start = time.time()
        self._set_direct_image_axes(self.tth)
        if self.mask is not None:
            self.intensity_ccd = ma.array(self.intensity_ccd, mask=self.mask).filled(0.0)
        print(str(datetime.datetime.now())[:-7],
    ...
    def get_ccd(self):
        if self.tth_ccd is None:
            return None, None, None
        else:
            return self.intensity_ccd, self.tth_ccd, self.chi_ccd
    ...
    def write_to_npy(self, chi_filen_wo_ext_in_temp):
        ...
        """
        f_tth = chi_filen_wo_ext_in_temp + '.tth.ccd.npy'
        f_chi = chi_filen_wo_ext_in_temp + '.chi.ccd.npy'
        f_int = chi_filen_wo_ext_in_temp + '.int.ccd.npy'

    def read_ccd_from_tempfile(self, temp_dir=None):
        ...
        if os.path.exists(tth_filen) and os.path.exists(azi_filen) and \
            ...
            self.tth_ccd = np.load(tth_filen)
            self.chi_ccd = np.load(azi_filen)
            self.intensity_ccd = np.load(int_filen)
            return True
        else:
            return False
    ...
    def make_temp_filenames(self, temp_dir=None):
        ...
        tth_filen = make_filename(self.img_filename, 'tth.ccd.npy',
                                  temp_dir=temp_dir)
        azi_filen = make_filename(self.img_filename, 'azi.ccd.npy',
                                  temp_dir=temp_dir)
        int_filen = make_filename(self.img_filename, 'int.ccd.npy',
                                  temp_dir=temp_dir)
        return tth_filen, azi_filen, int_filen

    def write_temp_ccdfiles(self, temp_dir):
        if not os.path.exists(temp_dir):
            os.makedirs(temp_dir)
        tth_filen, azi_filen, int_filen = self.make_temp_filenames(
            temp_dir=temp_dir)
        np.save(tth_filen, self.tth_ccd)
        np.save(azi_filen, self.chi_ccd)
        np.save(int_filen, self.intensity_ccd)
