...
class MapHistogramWidget(QtWidgets.QWidget):
    ...
    def _on_click(self, event):
        if event.inaxes != self.ax or event.xdata is None or self._xlims is None:
            return
    ...
    def _on_motion(self, event):
        if self._drag_target is None or self._xlims is None or event.xdata is None:
            return
        x = float(np.clip(event.xdata, self._xlims[0], self._xlims[1]))
        x0, x1 = self._xlims
